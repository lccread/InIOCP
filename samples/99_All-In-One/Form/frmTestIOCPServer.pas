...
type
  TFormTestIOCPServer = class(TForm)
    ...
    procedure InFileManager1RenameFile(Sender: TObject;
      Params: TReceiveParams; Result: TReturnResult);
    procedure InFileManager1ShareFile(Sender: TObject;
      Params: TReceiveParams; Result: TReturnResult);
    ...
    procedure InFileManager1TransmitRequest(Sender: TObject;
      Params: TReceiveParams; Result: TReturnResult);
...
begin
  ...
  if InClientManager1.Logined(DeleName, oSocket) then 
    InClientManager1.Delete(oSocket);
  ...
  case Result.Action of
    ...
    atDBExecStoredProc:
      if Result.ActResult = arOK then
        ...
        mmoClient.Lines.Add('查询失败: ' + Result.ErrMsg);
    atDBApplyUpdates:
      if Result.ActResult = arOK then
        mmoClient.Lines.Add('远程更新成功.')
      else
        mmoClient.Lines.Add('远程更新失败: ' + Result.ErrMsg);
    else
      if Result.ActResult = arOK then
        mmoClient.Lines.Add('执行成功.')
      else
        mmoClient.Lines.Add('执行失败: ' + Result.ErrMsg);
  end;
end;
...
//  if True then
    InFileManager1.SetWorkDir(Params.Socket, Params.Directory);
//  else
//    Result.ActResult := arFail;
end;

procedure TFormTestIOCPServer.InFileManager1ShareFile(Sender: TObject;
  Params: TReceiveParams; Result: TReturnResult);
begin
  // 把文件共享给其他部门用，结合数据库更好
  Result.ActResult := arOK;
...
procedure TFormTestIOCPServer.InFileManager1TransmitRequest(Sender: TObject;
  Params: TReceiveParams; Result: TReturnResult);
var
  oSocket: TIOCPSocket;
begin
  ...
  begin
    ...
    InFileManager1.TransmitFile(Params, oSocket);  // 会修改 ActResult
  end else
    Result.ActResult := arOffline;    // 客户离线
end;
...
procedure TFormTestIOCPServer.InMessageManager1GetFiles(Sender: TObject;
  Params: TReceiveParams; Result: TReturnResult);
begin
